...
void main() {
  ...
  testWidgets('desktop default constraints scale with window height', (
    tester,
  ) async {
  ...
  testWidgets(
    'a tall desktop window clamps the sheet to the absolute ceiling, a tall phone does not',
    (tester) async {
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.reset);
      ...
      unawaited(
        controller.show<void>(
          builder: (_) => const BottomSheetPageScaffold(
            title: 'Many',
            child: _FixedRowList(rowCount: 200),
          ),
        ),
      );
      await tester.pumpAndSettle();
  ...
  testWidgets(
    'a portrait tablet keeps the full 75% height — the ceiling is for resizable windows',
    (tester) async {
      ...
      expect(
        size.width,
        700,
        reason: 'the width cap still applies on a wide viewport',
      );
    },
  );

  testWidgets(
    'TV keeps the full 75% height — the ceiling is for resizable windows',
    (tester) async {
  ...
  testWidgets('the hostless modal fallback inherits the same sizing rules', (
    tester,
  ) async {
    PlatformDetector.debugSetIsDesktopOSOverride(true);
    TvDetectionService.debugSetAppleTVOverride(false);
  ...
  for (final anchor in [Alignment.bottomCenter, Alignment.topCenter]) {
    ...
    testWidgets(
      'a $label-anchored sheet keeps its anchored edge still while it grows',
      (tester) async {
        tester.view.physicalSize = const Size(1280, 800);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(tester.view.reset);
        ...
        unawaited(
          controller.show<void>(
            ...
            builder: (_) => const BottomSheetPageScaffold(
              title: 'Small',
              child: _FixedRowList(rowCount: 2),
            ),
          ),
        );
        await tester.pumpAndSettle();
        final animated = find.descendant(
          of: find.byType(OverlaySheetHost),
        ...
        unawaited(
          controller.push<void>(
            builder: (_) => const BottomSheetPageScaffold(
              title: 'Big',
              child: _FixedRowList(rowCount: 6),
            ),
          ),
        );
        ...
        if (isTop) {
          expect(
            ...
            headerTopBefore,
            reason: 'header is pinned to the top anchor',
          );
        } else {
          ...
          expect(
            tester.getBottomLeft(animated).dy,
            800,
            reason: 'bottom edge stays pinned to the viewport',
          );
        }
  ...
  testWidgets('popping back to a shorter page eases the sheet down', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(1280, 800);
    tester.view.devicePixelRatio = 1.0;
    ...
    expect(
      ...
      smallHeight + 160,
      reason: 'shrink must ease, not jump',
    );
    await tester.pump(const Duration(milliseconds: 90));
  ...
  testWidgets(
    'a short content-sized sheet still needs a deliberate drag to dismiss',
    (tester) async {
      tester.view.physicalSize = const Size(1280, 800);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.reset);
      ...
      expect(
        find.byType(BottomSheetPageScaffold),
        findsOneWidget,
        reason: '70px of drag must not dismiss',
      );
      ...
      expect(
        find.byType(BottomSheetPageScaffold),
        findsNothing,
        reason: '140px clears the floor',
      );
    },
  );

  testWidgets(
    'a sheet shorter than the dismiss floor stays dismissible by drag',
    (tester) async {
      tester.view.physicalSize = const Size(1280, 800);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.reset);
      ...
      unawaited(
        controller.show<void>(
          ...
          builder: (_) => const BottomSheetPageScaffold(
            title: 'One',
            child: _FixedRowList(rowCount: 1),
          ),
        ),
      );
      await tester.pumpAndSettle();
      ...
      expect(
        sheetHeight,
        lessThan(96 / 0.6),
        reason: 'the clamp must actually be the binding rule here',
      );
      ...
      expect(
        find.byType(BottomSheetPageScaffold),
        findsNothing,
        reason: 'clamped floor keeps a short sheet dismissible',
      );
    },
  );

  testWidgets(
    'sheet page shrinks to short content instead of filling the height cap',
    (tester) async {
      tester.view.physicalSize = const Size(1280, 800);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.reset);
      ...
      final sheetHeight = tester
          .getSize(find.byType(BottomSheetPageScaffold))
          .height;
      final headerHeight = tester
          .getSize(find.byType(BottomSheetHeader))
          .height;
  ...
  testWidgets('sheet page clamps to the height cap once content overflows', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(1280, 800);
    tester.view.devicePixelRatio = 1.0;
  ...
  testWidgets(
    'uneven row heights do not make the clamped sheet wobble while scrolling',
    (tester) async {
      tester.view.physicalSize = const Size(1280, 800);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.reset);
      ...
      await _pumpHostedSheet(
        ...
        const BottomSheetPageScaffold(
          title: 'Uneven',
          child: _FixedRowList(rowCount: 11, varyHeights: true),
        ),
      );
  ...
  testWidgets(
    'split columns size to the taller column and stretch the rule to match',
    (tester) async {
      tester.view.physicalSize = const Size(1280, 800);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.reset);
      ...
      await _pumpHostedSheet(
        tester,
        const SheetSplitColumns(
          start: _FixedRowList(rowCount: 2),
          end: _FixedRowList(rowCount: 3),
        ),
      );
      ...
      final rule = tester.getSize(
        find.descendant(
          of: find.byType(SheetSplitColumns),
  ...
  testWidgets(
    'replacing an open sheet adopts the new height instead of easing down from the old one',
    (tester) async {
      tester.view.physicalSize = const Size(1280, 800);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.reset);
  ...
  testWidgets(
    'onCloseStart fires immediately for closeAdaptive, barrier, and system back',
    (tester) async {
      var closeStarts = 0;
                      ...
                      OverlaySheetController.of(context).show<void>(
                        ...
                        builder: (sheetContext) => SizedBox(
                          ...
                          child: Center(
                            child: ElevatedButton(
                              onPressed: () =>
                                  OverlaySheetController.closeAdaptive(
                                    sheetContext,
                                  ),
  ...
  testWidgets(
    'pointer-opened sheet claims focus and handles Back before the screen',
    (tester) async {
  ...
  testWidgets(
    'pushAdaptive opens a root page on an idle host and returns its result',
    (tester) async {
                ...
                builder: (context) => Column(
                  children: [
                    ElevatedButton(
                      onPressed: () async {
                        final value =
                            await OverlaySheetController.pushAdaptive<String>(
                              context,
                              builder: (sheetContext) => SizedBox(
                                height: 120,
                                child: Column(
  ...
  testWidgets(
    'pushAdaptive pushes a nested page on an open host and pop restores the root',
    (tester) async {
                          ...
                          child: Column(
                            children: [
                              ...
                              ElevatedButton(
                                onPressed: () async {
                                  final value =
                                      await OverlaySheetController.pushAdaptive<
                                        String
                                      >(
                                        rootContext,
                              ...
                              ValueListenableBuilder<String>(
                                ...
                                builder: (_, value, _) =>
                                    Text('Nested result: $value'),
                              ),
                              ElevatedButton(
                                onPressed: () => OverlaySheetController.of(
                                  rootContext,
                                ).close(),
  ...
  group('opt-in canPop / onSystemBack', () {
    ...
    Future<void> pushHost(
      WidgetTester tester, {
      required bool? canPop,
      VoidCallback? onSystemBack,
    }) async {
      await tester.pumpWidget(
        MaterialApp(
          theme: ThemeData(platform: TargetPlatform.android),
    ...
    testWidgets(
      'canPop null installs no PopScope (system back pops the route)',
      (tester) async {
        ...
        expect(
          find.text('Open'),
          findsNothing,
          reason: 'route popped back to home',
        );
        expect(find.text('Push'), findsOneWidget);
      },
    );

    testWidgets('canPop true pops the route natively on system back', (
      tester,
    ) async {
    ...
    testWidgets('canPop false blocks the pop and runs onSystemBack', (
      tester,
    ) async {
      var backs = 0;
    ...
    testWidgets(
      'system back closes an open sheet instead of popping or running onSystemBack',
      (tester) async {
        var backs = 0;
        await pushHost(tester, canPop: false, onSystemBack: () => backs++);
        await tester.tap(find.text('Open'));
        ...
        expect(
          backs,
          0,
          reason: 'onSystemBack not called while a sheet was open',
        );
      },
    );

    testWidgets(
      'system back does not duplicate a handled TV key Back on an open sheet',
      (tester) async {
    ...
    testWidgets(
      'touch system back closes the sheet despite an unrelated handled mark',
      (tester) async {
        ...
        expect(
          find.text('SHEET'),
          findsNothing,
          reason: 'stale mark must not swallow the close',
        );
        expect(find.text('Open'), findsOneWidget, reason: 'screen not popped');
        expect(backs, 0);
      },
    ...
    testWidgets(
      'system back in a later frame is not mistaken for a duplicate TV key',
      (tester) async {
  ...
  Widget build(BuildContext context) {
    return ListView.builder(
      ...
      itemBuilder: (_, index) => SizedBox(
        height: varyHeights ? 40 + (index % 3) * 24 : 40,
...
Future<void> _slowDragDown(WidgetTester tester, double distance) async {
  final gesture = await tester.startGesture(
    tester.getCenter(find.byType(BottomSheetHeader)),
  );
