...
has_only_unchanged_upstream_native_format_drift() {
  local log=$1
  local official_tag=$2
  local path
  local paths=()

  grep -Fq '  FAIL  native formatting check failed' "$log" || return 1

  while IFS= read -r path; do
    [[ -n "$path" ]] && paths+=("$path")
  done < <(
    sed -n '/^==> native format$/,/^==> Dart analyzer$/p' "$log" |
      awk -v prefix="    $ROOT/" '
        index($0, prefix) == 1 {
...
ACCEPTED_FAILURES=0
if grep -Fq '  FAIL  formatting issues' "$CANDIDATE_LOG" &&
  has_only_unchanged_upstream_format_drift "$CANDIDATE_LOG" "$OFFICIAL_TAG"; then
  echo "::warning::Official $OFFICIAL_TAG contains every file reported by the Dart formatter unchanged; accepting the upstream baseline failure."
  ACCEPTED_FAILURES=$((ACCEPTED_FAILURES + 1))
fi
if has_only_unchanged_upstream_native_format_drift "$CANDIDATE_LOG" "$OFFICIAL_TAG"; then
  echo "::warning::Official $OFFICIAL_TAG contains every file reported by the native formatter unchanged; accepting the upstream baseline failure."
  ACCEPTED_FAILURES=$((ACCEPTED_FAILURES + 1))
fi
