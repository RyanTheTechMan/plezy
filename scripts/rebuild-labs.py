...
def read_json_at(repo: Path, ref: str, path: str) -> dict[str, Any]:
    ...
    if not isinstance(data, dict) or data.get("schema_version") != 1:
        raise RebuildError("manifest", "Feature manifest schema_version must be 1")
    ...
    for feature in features:
        ...
        if not isinstance(commits, list) or not commits or not all(isinstance(item, str) for item in commits):
...
def regenerate_translations(repo: Path) -> bool:
    ...
    generated = sorted((repo / "lib" / "i18n").glob("strings*.g.dart"))
    if generated:
        git(repo, "add", "--", *(str(path.relative_to(repo)) for path in generated))
...
def rebuild(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for feature in manifest["features"]:
        ...
        for source_commit in feature["commits"]:
            try:
                source_commit = rev_parse(repo, source_commit)
            except RuntimeError as error:
                raise RebuildError("feature", str(error), feature=feature_id) from error
        ...
        if applied_commits:
            report["applied_features"].append({"id": feature_id, "commits": applied_commits})
        if skipped_commits:
            report["skipped_official_features"].append({"id": feature_id, "commits": skipped_commits})
