...
LINUX_PACKAGES = "linux/packaging/build-packages.py"
...
def brand_labs_linux_packaging(contents: str) -> str:
    """Apply Labs-owned package metadata to current upstream packaging code."""
    metadata = '''
...
'''
    version_override = '''    labs_version = os.environ.get("LABS_VERSION")
    if labs_version:
        return labs_version
'''
    iteration = '''    iteration = os.environ.get("LABS_PACKAGE_ITERATION", "1")
    if distro == "pacman":
...
'''
    output_line = (
        '    output_file = OUTPUT_DIR / f"{METADATA[\'name\']}-linux-'
        '{ARCH_SUFFIX}.{config[\'ext\']}"\n'
    )

    blocks = (metadata, version_override, iteration, '        "--iteration", iteration,\n')
    present = [block in contents for block in blocks]
    if all(present):
        return contents
    if any(present):
        raise ValueError("Linux packaging contains only part of the Labs metadata")

    metadata_match = re.search(r"(?ms)^(METADATA = \{\n.*?^\}\n)", contents)
    version_match = re.search(
        r'(?m)^(def get_version\(\) -> str:\n    """[^\n]+"""\n)',
        contents,
    )
    if metadata_match is None or version_match is None or contents.count(output_line) != 1:
        raise ValueError("official Linux packaging structure was not found")
    if contents.count('        "--iteration", "1",\n') != 1:
        raise ValueError("official Linux package iteration declaration was not found")

    contents = (
        contents[:metadata_match.end()]
        + metadata
        + contents[metadata_match.end():]
    )
    version_match = re.search(
    ...
    contents = (
        contents[:version_match.end()]
        + version_override
        + contents[version_match.end():]
    )
    output_index = contents.index(output_line) + len(output_line)
    contents = contents[:output_index] + iteration + contents[output_index:]
...
def resolve_labs_base_conflicts(repo: Path, files: list[str]) -> bool:
    ...
    if LINUX_PACKAGES in files:
        supported.add(LINUX_PACKAGES)
    ...
    if LINUX_PACKAGES in files:
        try:
            git(repo, "checkout", "--ours", "--", LINUX_PACKAGES)
            packaging = repo / LINUX_PACKAGES
            packaging.write_text(
                brand_labs_linux_packaging(packaging.read_text(encoding="utf-8")),
                encoding="utf-8",
            )
            git(repo, "add", "--", LINUX_PACKAGES)
        except (OSError, RuntimeError, ValueError):
            return False
