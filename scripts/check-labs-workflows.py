...
main = require(
    ...
    [
        ...
        'if [[ "$default_branch" != labs ]]',
        "actions/workflows?per_page=100",
        'git cat-file -e "origin/labs:$path"',
        "is absent from Labs and not registered",
        ...
        "scripts/fetch_linux_libmpv.py --dest libmpv-prefix",
        "Verify Windows bundle contents",
        "Install Flatpak packaging dependencies",
        "bubblewrap_0.9.0-1ubuntu0.3_arm64.deb",
        "dce3a2e5f0302eab717a64fe9c9411fe2d40469e2a7946e0f2ce1420a61025c3",
        "https://ports.ubuntu.com/ubuntu-ports/pool/main/b/bubblewrap/",
        'flatpak_packages=("$bubblewrap_deb" "${flatpak_packages[@]}")',
        'dpkg --compare-versions "$bubblewrap_version" ge 0.9.0-1ubuntu0.1',
        'python3 linux/packaging/build-flatpak.py --bundle "$BUNDLE_DIR" --arch ${{ matrix.arch }} --output "$GITHUB_WORKSPACE"',
        "Verify installed Flatpak runtime dependencies",
        "dbus-run-session -- python3 linux/packaging/check-flatpak.py --arch ${{ matrix.arch }}",
        "plezy-linux-${{ matrix.arch }}.flatpak",
...
if "linux/packaging/build-libmpv.sh" in main:
    raise SystemExit("Labs orchestration must not call the removed Linux libmpv build script")
if main.count("plezy-linux-${{ matrix.arch }}.flatpak") < 3:
    raise SystemExit("Labs Flatpak must be runtime-checked, attested, and uploaded with each Linux artifact set")
...
require(
    ...
    [
        ...
        "Included in Labs, not planned for Plezy",
        "Merged into Plezy",
