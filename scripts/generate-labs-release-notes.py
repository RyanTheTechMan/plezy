...
UPSTREAM_UPDATE_HEADINGS = {
    "not_planned": "Included in Labs, not planned for Plezy",
    "merged": "Labs features merged into Plezy",
}
...
def upstream_update_lines(manifest: dict[str, Any]) -> dict[str, list[str]]:
    grouped = {status: [] for status in UPSTREAM_UPDATE_HEADINGS}
    updates = manifest.get("upstream_updates", [])
    if not isinstance(updates, list):
        raise ValueError("Feature manifest upstream_updates must be a list")

    seen_prs: set[str] = set()
    for update in updates:
        if not isinstance(update, dict):
            raise ValueError("Every upstream update must be an object")
        status = update.get("status")
        if status not in grouped:
            raise ValueError(f"Invalid upstream update status: {status}")
        description = update.get("description")
        if not isinstance(description, str) or not description.strip():
            raise ValueError("Every upstream update must have a description")
        upstream_pr = update.get("upstream_pr")
        if not isinstance(upstream_pr, str) or not (match := UPSTREAM_PR.fullmatch(upstream_pr)):
            raise ValueError("Every upstream update must have a valid upstream PR")
        if upstream_pr in seen_prs:
            raise ValueError(f"Duplicate upstream update PR: {upstream_pr}")
        seen_prs.add(upstream_pr)
        grouped[status].append(
            f"{description.strip()} ([upstream PR #{match.group(1)}]({upstream_pr}))"
        )
    return grouped
...
def generate_release_notes(
    ...
    updates = upstream_update_lines(manifest)
    for status, heading in UPSTREAM_UPDATE_HEADINGS.items():
        if not updates[status]:
            continue
        lines.extend(["", f"## {heading}", ""])
        lines.extend(f"- {description}" for description in updates[status])
