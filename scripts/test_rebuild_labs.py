...
class RebuildLabsTest(unittest.TestCase):
    ...
    def test_rebrands_current_upstream_linux_packaging(self) -> None:
        upstream = '''import os
...
'''
        ...
        branded = REBUILD_LABS.brand_labs_linux_packaging(upstream)

        self.assertIn('if os.environ.get("LABS_VERSION"):', branded)
        self.assertIn('labs_version = os.environ.get("LABS_VERSION")', branded)
        self.assertIn('iteration = os.environ.get("LABS_PACKAGE_ITERATION", "1")', branded)
        self.assertIn('"--iteration", iteration', branded)
        self.assertIn('return "2.19.0"', branded)
        self.assertEqual(REBUILD_LABS.brand_labs_linux_packaging(branded), branded)

    def test_refuses_partial_labs_linux_packaging(self) -> None:
        with self.assertRaisesRegex(ValueError, "only part"):
            REBUILD_LABS.brand_labs_linux_packaging(
